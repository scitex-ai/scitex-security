"""`scitex-security skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-security/` directory directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-security/` directory."""
    import scitex_security

    pkg_dir = Path(scitex_security.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-security/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-security.

    \b
    Examples:
      $ scitex-security skills list
      $ scitex-security skills get 01_installation
      $ scitex-security skills install                  # → ~/.scitex/dev/skills/scitex-security/
      $ scitex-security skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_list(as_json: bool) -> None:
    ...
    Example:
      $ scitex-security skills list
      $ scitex-security skills list --json
    """
    root = _skills_root()
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-security skills get 01_installation
      $ scitex-security skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-security/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-security/` dir to
    ~/.scitex/dev/skills/scitex-security/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-security skills install
      $ scitex-security skills install --claude-symlink
      $ scitex-security skills install --no-link --dest /tmp/scitex-security-skills
    """
