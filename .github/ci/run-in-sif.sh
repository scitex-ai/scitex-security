...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-security's.
...
# scitex-dev's deps), so only scitex-security's own thin dep set is fetched per
# run.
#
...
export TMPDIR="/tmp/ci-scitex_security-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# Install scitex-security + its [all,dev] extras WITH deps into the writable
# target. Fallback chain so a packaging hiccup in an optional extra doesn't
# strand CI: [all,dev] → [dev] → bare. uv first (fast resolver), pip safety net.
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_security --cov-report=xml --cov-report=term \
    -p no:cacheprovider
